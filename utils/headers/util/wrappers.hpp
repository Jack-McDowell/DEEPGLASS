...
public:
	...
	operator LPCRITICAL_SECTION() const { return static_cast<LPCRITICAL_SECTION>(counter.get()); }
};
...
public:
	...
	LPVOID operator+(size_t offset){
		return offset < AllocationSize ? reinterpret_cast<LPVOID>(reinterpret_cast<ULONG_PTR>(pointer) + offset) : nullptr;
	}
