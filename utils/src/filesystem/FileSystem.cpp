...
namespace FileSystem {
    ...
    std::optional<std::wstring> File::CalculateHashType(HashType hashType) const {
        ...
        case HashType::SHA256_HASH: success = false; // CryptCreateHash(hProv, CALG_SHA_256, 0, 0, &hash); break;
        }
    ...
    File::File(IN const std::wstring& path, ACCESS_MASK permissions) : hFile{ nullptr } {
        if(!path.length()) {
            bFileExists = false;
            bWriteAccess = false;
            bReadAccess = false;
        ...
        if(FilePath.at(0) == L'\\') {
            ...
            InitializeObjectAttributes(&attributes, &UnicodeName, OBJ_KERNEL_HANDLE | OBJ_CASE_INSENSITIVE, nullptr,
                                       nullptr);
            NTSTATUS Status{ Linker::NtCreateFile(&hFile, permissions, &attributes, &IoStatus, nullptr,
            ...
            if(NT_SUCCESS(Status)) {
                ...
                Status = Linker::NtCreateFile(&hFile, GENERIC_READ, &attributes, &IoStatus, nullptr,
                                              FILE_ATTRIBUTE_NORMAL, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN, FILE_SEQUENTIAL_ONLY,
                                              nullptr, 0);
                if(NT_SUCCESS(Status)) {
                    this->hFile = hFile;
                    bFileExists = true;
            ...
            hFile = CreateFileW(FilePath.c_str(), permissions, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
