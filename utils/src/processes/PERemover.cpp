...
bool PERemover::RemoveImage(){
    ...
    if(CheckThreads() && AdjustPointers() && WipeMemory()){
        LOG_INFO(1, "Successfully removed image at " << lpBaseAddress << " from process with PID " << GetProcessId(hProcess));
        return true;
    }
...
bool PERemover::WalkThreadBack(const HandleWrapper& hThread, DWORD dwTID){
    ...
    if(AddressIsInRegion(reinterpret_cast<LPVOID>(context.Eip))){
        if(TerminateThread(hThread, 0)){
            LOG_INFO(1, "Thread with TID " << dwTID << " was executing malicious code at " << reinterpret_cast<LPVOID>(context.Eip) << " and was terminated");
            return true;
        } else {
