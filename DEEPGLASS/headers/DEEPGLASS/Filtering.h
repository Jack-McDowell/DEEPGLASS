...
namespace DEEPGLASS{
    ...
    /*!
     * \brief Checks if a memory region is a PE by reading its contents and comparing to the PE32+ standard.
     *
     * \param[in] file The memory region to check
     *
     * \return True if the memory referrenced conforms to the PE32+ standard; false otherwise.
     */
    bool IsPEData(_In_ const MemoryWrapper<>& memory);
